...
def createNetwork():
    ...
    # input layer
    s = tf.placeholder("float", [None, 64, 64, 2])
...
def playGame():
    ...
    sess,s,a,y,readout,train_step,epsilon,t,D,s_t = q_net_prepare(sess,s,readout)

    trainNetwork(sess,s,a,y,readout,train_step,epsilon,t,D)



def trainNetwork(sess,saver,s,a,y,readout,train_step,epsilon,t,D,s_0,seg_get_state):
    ...
    while count < 1:
        ...
        s_t1, r_t, terminal = seg_get_state(action_index)
...
def segnet_stat(action,segnet,image_batch,label_batch):
    #x_t1_colored, r_t, terminal = game_state.frame_step(a_t)


    action_set = [1.0,0.1,10]

    seg_vars = segnet.variables()

    seg_vars_copy = copy(seg_vars)

    image,label = image_batch[0],label_batch[0]



    return status_t1,r_t,terminal
...
# def trainNetwork(sess,s,a,y,readout,train_step,epsilon,t,D,s_t):
